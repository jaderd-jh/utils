...
// 创建加密原子（第三个参数为 true）
const credentialsAtom = atomWithLocal<SensitiveData>('credentials', {
  apiKey: '',
  apiSecret: '',
  accessToken: ''
}, true)
...
function useTranslation() {
  ...
  const t = (key: string): string => {
    return translations[config.language][key]
      || translations[config.fallbackLanguage][key]
      || key
  }
...
// 认证状态（加密存储）
const authAtom = atomWithLocal<AuthState>('auth', {
  user: null,
  token: '',
  refreshToken: '',
  expiresAt: 0
...
function ActionButtonA() {
  ...
  return (
    <button onClick={() => addNotification('操作 A 完成', 'success')}>
      执行操作 A
    </button>
  )
}
...
function ActionButtonB() {
  ...
  return (
    <button onClick={() => addNotification('操作 B 失败', 'error')}>
      执行操作 B
    </button>
  )
}
